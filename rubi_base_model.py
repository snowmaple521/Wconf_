...
class BaseModel(nn.Module):
    ...
    def forward(self, v, q, labels, bias,v_mask):
        ...
        q_pred=self.c_1(q_emb.detach()) #rubi add this code MLP

        q_out=self.c_2(q_pred) #rubi add this code2 Linear
        # rubi_logits = logits + torch.sigmoid(q_out)
        if labels is not None:
            rubi_logits=logits + torch.sigmoid(q_pred)
            #rubi add this code for only_q loss + rubi_logits
            # loss=F.binary_cross_entropy_with_logits(rubi_logits, labels)+3.4*compute_self_loss(rubi_logits, labels)
            loss = F.binary_cross_entropy_with_logits(rubi_logits, labels) + F.binary_cross_entropy_with_logits(q_out,
                                                                                                                labels)
