...
class VQAFeatureDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.name=='train':
            ...
            if "bias" in entry:
                return features, ques, target,entry["bias"],train_hint,type_mask,ques_mask,q_id

            else:
                return features, ques,target, 0,train_hint,type_mask,ques_mask,q_id
        else:
            test_hint=torch.tensor(self.test_hintsocre[str(q_id)])

            if "bias" in entry:
                return features, ques,target, entry["bias"],test_hint,ques_mask,q_id
            else:
                return features, ques, target, 0,test_hint,ques_mask,q_id
