...
def _create_entry(img_idx, question, answer):
    ...
    entry = {
        'question_type':answer['question_type'].capitalize(),
        'question_notype':question['question'].replace(answer['question_type'].capitalize(),'',1),
...
def _load_dataset(dataroot, name, img_id2val, dataset):
    ...
    """
    if dataset=='cpv2':
      # answer_path = os.path.join(dataroot, 'cp-cache', 'cp_v2_%s_target_plus.pkl' % name)
      answer_path = os.path.join(dataroot, 'cp-cache', 'new_%s_target_entroy_css_5w.pkl' % name)
      name = "train" if name == "train" else "test"
      # question_path = os.path.join(dataroot, 'vqacp_v2_%s_questions_plus.json' % name)
      question_path = os.path.join(dataroot, 'new_%s_questions_entroy_css_5w.json' % name)
      with open(question_path) as f:
        questions = json.load(f)
      question_notype_path = os.path.join(dataroot, 'vqacp_v2_%s_questions_notype.json' % name)
      with open(question_notype_path) as f:
        questions_notype= json.load(f)
    elif dataset=='cpv1':
    ...
    # questions_notype.sort(key=lambda x: x['question_id'])
    # if name == 'train':
...
class VQAFeatureDataset(Dataset):
    def __init__(self, name, dictionary, dataroot='data', dataset='cpv2',
        ...
        self.name=name
        if dataset=='cpv2':
            with open('data/new_cpv2_hintscore_entroy_5w.json', 'r') as f:
                self.train_hintscore = json.load(f)
            with open('data/test_cpv2_hintscore_entroy.json', 'r') as f:
    ...
    def tokenize(self, max_length=14):
        ...
        for entry in tqdm(self.entries, ncols=100, desc="tokenize"):
            ...
            tokens_noty = self.dictionary.tokenize(entry['question_notype'], False)
            tokens_noty = tokens_noty[:max_length]
            # tokens_ty = self.dictionary.tokenize(entry['question_type'], False)
            # tokens_ty = tokens_ty[:max_length]
            if len(tokens_noty) < max_length:
                # Note here we pad to the back of the sentence
                padding = [1] * \
                          (max_length - len(tokens_noty))
                tokens_noty = tokens_noty + padding
            # if len(tokens_ty) < max_length:
            #     # Note here we pad to the back of the sentence
            #     padding = [self.dictionary.padding_idx] * \
            #               (max_length - len(tokens_ty))
            #     tokens_ty = tokens_ty + padding
            ...
            entry['q_noty_token'] = tokens_noty
            # entry['q_ty_token'] = tokens_ty
    ...
    def tensorize(self):
        for entry in tqdm(self.entries, ncols=100, desc="tensorize"):
            ...
            question_noty = torch.from_numpy(np.array(entry['q_noty_token']))
            # question_ty = torch.from_numpy(np.array(entry['q_ty_token']))
            entry['q_token'] = question
            entry['q_token_mask']=question_mask
            entry['q_noty_token']=question_noty
            # entry['q_ty_token']=question_ty


            answer = entry['answer']
            labels = np.array(answer[u'labels'])
            scores = np.array(answer[u'scores'], dtype=np.float32)
            if len(labels):
                labels = torch.from_numpy(labels)
                scores = torch.from_numpy(scores)
                entry['answer'][u'labels'] = labels
                entry['answer'][u'scores'] = scores
            else:
    ...
    def __getitem__(self, index):
        ...
        q_noty=entry['q_noty_token']
        # q_ty=entry['q_ty_token']
        answer = entry['answer']
        ...
        if self.name=='train':
            ...
            if "bias" in entry:
                return features, ques, target,entry["bias"],train_hint,q_noty,ques_mask,q_id

            else:
                return features, ques,target, 0,train_hint,q_id
        else:
